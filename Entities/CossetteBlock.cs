...
{
    [CustomEntity("DzhakeHelper/CossetteBlock")]

    [Tracked(true)]
    public class CossetteBlock : Solid
    {
        ...
        {
            private CossetteBlock block;

            private Color color;

            public BoxSide(CossetteBlock block, Color color)
            {
                this.block = block;
                this.color = color;
            }
        ...
        private List<CossetteBlock> group;
        ...
        public CossetteBlock(EntityData data, Vector2 offset, EntityID id)
        ...
        public override void Awake(Scene scene)
        {
            ...
            if (group == null)
            {
                ...
                foreach (CossetteBlock item in group)
                {
                    if (item.Left < num)
                    {
                        num = item.Left;
                ...
                foreach (CossetteBlock item2 in group)
                {
        ...
        private void FindInGroup(CossetteBlock block)
        {
        ...
        {
            List<MTexture> atlasSubtextures = GFX.Game.GetAtlasSubtextures("objects/DzhakeHelper/cossetteBlock/pressed");
            pressed.Add(CreateImage(x, y, tx, ty, atlasSubtextures[Index % atlasSubtextures.Count]));
            solid.Add(CreateImage(x, y, tx, ty, GFX.Game["objects/DzhakeHelper/cossetteBlock/solid"]));
        }
        ...
        private void UpdateVisualState()
        {
            ...
            foreach (CossetteBlock item3 in group)
            {
        ...
        public CossetteBlock(EntityData data, Vector2 offset)
